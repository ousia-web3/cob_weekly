...
class EmailService:
    ...
    def send_email(self, subject, html_content, recipients=None, html_path=None):
        ...
        try:
            ...
            # send_message는 msg 객체의 헤더情報を 기반으로 발송하지만,
            # 명시적으로 수신자 리스트를 넘겨주는 것이 안전함 (BCC 등이 포함될 경우 등 고려)
            # 여기서는 To에 모두 표기되므로 msg['To']와 실제 수신자가 일치함
            server.send_message(msg)

            success_count = len(recipients)
            fail_count = 0

            result_message = f"발송 완료: 총 {success_count}명에게 일괄 발송 성공"
            status = "success"
            ...
            self.add_history_entry(
                subject, recipients, status, result_message, html_path
            )
    ...
    def resend_email(self, history_id):
        ...
        try:
            ...
            if use_current_recipients:
                logger.info(
                    f"재발송: 저장된 수신자 정보가 유효하지 않아 현재 활성 수신자 목록을 사용합니다. (ID: {history_id})"
                )
                all_recipients = self.get_recipients()
                recipients = [
                    r["email"] for r in all_recipients if r.get("active", True)
                ]

                if not recipients:
                    return (
                        False,
                        "발송할 수신자가 없습니다. (저장된 수신자 없음 & 현재 수신자 없음)",
                    )
    ...
    def sync_to_env(self):
        ...
        try:
            ...
            return (
                True,
                f"{len(active_emails)}명의 수신자가 .env 파일에 동기화되었습니다.",
            )
        except Exception as e:
    ...
    def import_from_env(self, overwrite=False):
        ...
        try:
            ...
            if overwrite:
                ...
                return (
                    True,
                    f"{len(env_emails)}명의 수신자를 가져왔습니다. (기존 목록 삭제됨)",
                )
            else:
                # 기존 목록에 추가 (중복 제외)
                ...
                return (
                    True,
                    f"{added_count}명의 새로운 수신자를 추가했습니다. (중복 {len(env_emails) - added_count}명 제외)",
                )
